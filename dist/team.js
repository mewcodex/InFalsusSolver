...
try{
 ...
 const [team,data]=await Promise.all([loadoutFiles[choice],'data.json'].map(async p=>{const r=await fetch(p+'?v=team-notes-20260922');if(!r.ok)throw Error('方案文件加载失败');return r.json()}));
 ...
 const render=()=>{document.getElementById('team-grid').innerHTML=team.cards.map(card=>{const recipe=data.recipes.find(r=>r.id===card.id),s=card.crafting.score;return `<a class="team-card" href="index.html?team=${card.position}&loadout=${choice}"><div class="team-card-head"><p style="color:${colors[card.color]}">第 ${card.position} 阶段 · 成品${colorNames[card.color]}色</p><h2>${escape(recipe.name)}</h2><p>威力 ${num(card.power)} ／ 耐力 ${num(card.fortitude)}</p></div>${teamPreview(recipe,card,recipe.name)}<div class="team-card-body"><p>${card.slots} 特质槽 · 额外左 ${card.left}／右 ${card.right}<br>粒子 ${card.crafting.placements.length}/${s.limit} · ${s.total} 次惩罚（已计入属性）</p><ul>${card.traits.length?card.traits.map(t=>`<li>${escape(t.name)}</li>`).join(''):'<li>无装备特质</li>'}</ul><span class="open-plan">放大查看完整方案 ↗</span></div></a>`}).join('')};render();
