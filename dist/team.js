...
try{
 ...
 const [team,data]=await Promise.all([loadoutFiles[choice],'data.json'].map(async p=>{const r=await fetch(p+'?v=stack3-20260919');if(!r.ok)throw Error('方案文件加载失败');return r.json()}));
 ...
 if(choice==='storm-score'){
  ...
  context='无常的风暴 · 以成功通关后的遭遇分数最高为目标；已计入敌方技能、红色限制、回复和生存。此处展示当前最高已知分数方案。';
