...
try{
 ...
 if(choice==='storm-score'){
  ...
  document.getElementById('team-conditions').insertAdjacentHTML('beforeend',`<h3>粒子 995 · 11 级曲参考</h3><p>保持本页拼法、颜色和技能，全部粒子效能改为 995、谱面等级改为 11：预计遭遇分 <strong>${num(r.score)}</strong>，攻击率 ${num(r.dp*100/(100+r.he)*100)}、防御率 ${num(10000*(100+r.hp)/r.de)}；最低血量 ${r.minHp.toFixed(2)}%，结束血量 ${r.phases.at(-1).playerHp.toFixed(2)}%。按双方准确率 100%、五阶段等物量参考模型计算；具体谱面的整数判定数会带来小幅差异。</p><p>当前配卡已使用最新合并解库重新搜索，并检查单卡替换、特质逐项调整、同卡特质顺序及双卡候选替换，未找到更高分组合。具体复查快照与评估次数见下方；尚未穷举所有配卡和特质组合。</p>`);
 }
 if(choice==='balanced'){
