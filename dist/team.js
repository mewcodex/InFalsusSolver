...
try{
 ...
 const [team,data]=await Promise.all([loadoutFiles[choice],'data.json'].map(async p=>{const r=await fetch(p+'?v=ensemble-20260919');if(!r.ok)throw Error('方案文件加载失败');return r.json()}));
